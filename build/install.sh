...
Now you should access CLI script: $ pypackage-example --help
Alternatively you can access CLI script via poetry run: $ poetry run pypackage-example --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_PYPACKAGE_EXAMPLE_COMPLETE=source_bash pypackage-example)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_PYPACKAGE_EXAMPLE_COMPLETE=source_zsh pypackage-example)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_PYPACKAGE_EXAMPLE_COMPLETE=source_fish pypackage-example)' >> ~/.config/fish/completions/pypackage-example.fish
EOF